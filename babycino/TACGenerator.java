...
public class TACGenerator extends MiniJavaBaseVisitor<TACBlock> {
    ...
    public TACBlock visitExpBinOp(MiniJavaParser.ExpBinOpContext ctx) {
        ...
        if (op.equals("==")) {
            ...
            result.add(TACOp.binop(reg1, expr1.getResult(), expr2.getResult(), 2));
        /* 2 is for subtraction --- from TACOp.java*/
            result.add(TACOp.jz(reg1, lbl));
            result.add(TACOp.immed(reg1, 0));
            result.add(TACOp.jmp(end));
            result.add(TACOp.label(lbl));
            result.add(TACOp.immed(reg1, 1));
            result.add(TACOp.label(end));
            
            result.setResult(reg1);            
            return result;
        }
