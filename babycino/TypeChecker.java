    ...
    public void exitExpBinOp(MiniJavaParser.ExpBinOpContext ctx) {
        ...
        switch (op) {
            ...
            case "&&":
                ...
                break;
            case "==":
             this.check(lhs.isBoolean() || lhs.isInt(), ctx, "Expected int or boolean for ==; actual type: " + lhs);
             if(lhs.isBoolean()){
                this.check(rhs.isBoolean(), ctx, "Expected boolean as 2nd argument to ==; actual type: " + rhs);
             }
             if(lhs.isInt()){
                this.check(rhs.isInt(), ctx, "Expected int as 2nd argument to ==; actual type: " + rhs);
             }
            default:
